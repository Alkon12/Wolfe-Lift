export const translations = {
  en: {
    nav: {
      home: "Home",
      equipment: "Equipment",
      gallery: "Gallery",
      services: "Services",
      contact: "Contact",
      phone: "(626) 422-2271",
      quote: "Get Quote",
    },
    hero: {
      headline: "Professional Scissor Lift Rentals",
      subheadline:
        "Serving Los Angeles County and surrounding areas with reliable equipment and same-day delivery",
      mainTitle1: "EMERGENCY SATURDAY",
      mainTitle2: "AND SUNDAY DELIVERY",
      mainTitle3: "AVAILABLE",
      qualityEquipment: "Quality Equipment",
      expertService: "Expert Service",
      sameDayDelivery: "Same-Day Delivery",
      saveTimeMoney: "Save time and money",
      reachHeights: "reach new heights",
      forProjects: "for all your projects",
      cta1: "Get Instant Quote",
      cta2: "Call Now",
    },
    services: {
      title: "Our Equipment",
      subtitle: "Professional grade scissor lifts for any job site",
      lift12: {
        title: "12ft Scissor Lift",
        specs: "12ft weight capacity 500 lb",
        rate: "Our Rates are",
      },
      lift19: {
        title: "19ft Scissor Lift",
        specs: "19ft weight capacity 500 lb",
        rate: "Our Rates are",
      },
      lift26: {
        title: "26ft Scissor Lift",
        specs: "26ft weight capacity 750 lb",
        rate: "Our Rates are",
      },
      lift32: {
        title: "32ft Scissor Lift",
        specs: "32ft weight capacity 1000 lb",
        rate: "Our Rates are",
      },
    },
    whyChoose: {
      title: "Why Choose Wolfe Lift?",
      subtitle:
        "Trusted by contractors throughout Los Angeles County for over 15 years",
      point1: {
        title: "Same-Day Delivery",
        desc: "Equipment delivered to your job site within hours",
      },
      point2: {
        title: "Affordable Delivery",
        desc: "Delivery starts at $150 - Pickup is FREE",
      },
      point3: {
        title: "Quality Equipment",
        desc: "Well-maintained professional equipment ready for any job",
      },
      point4: {
        title: "Competitive Rates",
        desc: "Best prices in Los Angeles County with no hidden fees",
      },
    },
    contact: {
      title: "Get Your Equipment Today",
      subtitle: "Quick quote • Same-day delivery • Professional service",
      form: {
        name: "Company Name",
        phone: "Phone Number",
        equipment: "Equipment Needed",
        date: "Start Date",
        submit: "Request Quote",
        placeholderCompany: "ABC Construction Co.",
        placeholderPhone: "(626) 422-2271",
        selectEquipment: "Select Equipment",
        lift19Option: "19' Scissor Lift",
        lift26Option: "26' Scissor Lift",
        lift32Option: "32' Scissor Lift",
        lift12Option: "12ft Scissor Lift",
        multipleOption: "Multiple Units",
      },
      info: {
        address: "Los Angeles",
        hours: "Mon-Fri: 6AM-6PM | Sat: 7AM-3PM",
      },
      callUsNow: "Call Us Now",
      emergencyRentals: "Available for emergency rentals",
      visitLocation: "Visit Our Location",
      serviceArea: "Service Area",
      serviceAreaDesc:
        "We proudly serve Los Angeles County and surrounding areas including:",
    },
    footer: {
      tagline: "Professional Equipment. Reliable Service. Fair Prices.",
      companyName: "Wolfe Lift",
      companyTagline: "Rentals & Transport",
      quickLinks: "Quick Links",
      equipmentRentals: "Equipment Rentals",
      contactUs: "Contact Us",
      contactTitle: "Contact",
      copyright: "© 2011 Wolfe Lift Rentals & Transport. All rights reserved.",
      license: "CA License #123456",
      links: {
        about: "About Us",
        safety: "Safety",
        terms: "Terms",
        privacy: "Privacy",
      },
    },
    mobile: {
      callNow: "Call Now",
      getQuote: "Get Quote",
    },
    loading: {
      elevatingProjects: "Elevating your projects...",
    },
  },
  es: {
    nav: {
      home: "Inicio",
      equipment: "Equipos",
      gallery: "Galería",
      services: "Servicios",
      contact: "Contacto",
      phone: "(626) 422-2271",
      quote: "Cotizar",
    },
    hero: {
      headline: "Renta de Plataformas Elevadoras",
      subheadline:
        "Sirviendo el Condado de Los Ángeles y áreas circundantes con equipo confiable y entrega el mismo día",
      mainTitle1: "ENTREGA DE EMERGENCIA",
      mainTitle2: "SÁBADO Y DOMINGO",
      mainTitle3: "DISPONIBLE",
      qualityEquipment: "Equipo de Calidad",
      expertService: "Servicio Experto",
      sameDayDelivery: "Entrega el Mismo Día",
      saveTimeMoney: "Ahorre tiempo y dinero",
      reachHeights: "alcance nuevas alturas",
      forProjects: "para todos sus proyectos",
      cta1: "Cotizar Ahora",
      cta2: "Llamar Ahora",
    },
    services: {
      title: "Nuestro Equipo",
      subtitle:
        "Plataformas elevadoras de grado profesional para cualquier obra",
      lift12: {
        title: "Plataforma de 12ft",
        specs: "12ft capacidad de peso 500 lb",
        rate: "Nuestras Tarifas son",
      },
      lift19: {
        title: "Plataforma de 19ft",
        specs: "19ft capacidad de peso 500 lb",
        rate: "Nuestras Tarifas son",
      },
      lift26: {
        title: "Plataforma de 26ft",
        specs: "26ft capacidad de peso 750 lb",
        rate: "Nuestras Tarifas son",
      },
      lift32: {
        title: "Plataforma de 32ft",
        specs: "32ft capacidad de peso 1000 lb",
        rate: "Nuestras Tarifas son",
      },
    },
    whyChoose: {
      title: "¿Por Qué Elegir Wolfe Lift?",
      subtitle:
        "Confianza de contratistas del Condado de Los Ángeles por más de 15 años",
      point1: {
        title: "Entrega el Mismo Día",
        desc: "Equipo entregado a su obra en horas",
      },
      point2: {
        title: "Entrega Económica",
        desc: "Entrega desde $150 - Recogida GRATIS",
      },
      point3: {
        title: "Equipo de Calidad",
        desc: "Equipo profesional bien mantenido listo para cualquier trabajo",
      },
      point4: {
        title: "Precios Competitivos",
        desc: "Mejores precios en el Condado de Los Ángeles sin cargos ocultos",
      },
    },
    contact: {
      title: "Obtenga Su Equipo Hoy",
      subtitle:
        "Cotización rápida • Entrega el mismo día • Servicio profesional",
      form: {
        name: "Nombre de la Empresa",
        phone: "Número de Teléfono",
        equipment: "Equipo Necesario",
        date: "Fecha de Inicio",
        submit: "Solicitar Cotización",
        placeholderCompany: "Constructora ABC Co.",
        placeholderPhone: "(626) 422-2271",
        selectEquipment: "Seleccionar Equipo",
        lift19Option: "Plataforma de 19'",
        lift26Option: "Plataforma de 26'",
        lift32Option: "Plataforma de 32'",
        lift12Option: "Plataforma de 12ft",
        multipleOption: "Múltiples Unidades",
      },
      info: {
        address: "Los Angeles",
        hours: "Lun-Vie: 6AM-6PM | Sáb: 7AM-3PM",
      },
      callUsNow: "Llámanos Ahora",
      emergencyRentals: "Disponible para rentas de emergencia",
      visitLocation: "Visite Nuestra Ubicación",
      serviceArea: "Área de Servicio",
      serviceAreaDesc:
        "Servimos con orgullo el Condado de Los Ángeles y áreas circundantes incluyendo:",
    },
    footer: {
      tagline: "Equipo Profesional. Servicio Confiable. Precios Justos.",
      companyName: "Wolfe Lift",
      companyTagline: "Rentals & Transport",
      quickLinks: "Enlaces Rápidos",
      equipmentRentals: "Renta de Equipos",
      contactUs: "Contáctanos",
      contactTitle: "Contacto",
      copyright:
        "© 2024 Wolfe Lift Rentals & Transport. Todos los derechos reservados.",
      license: "Licencia CA #123456",
      links: {
        about: "Nosotros",
        safety: "Seguridad",
        terms: "Términos",
        privacy: "Privacidad",
      },
    },
    mobile: {
      callNow: "Llamar Ahora",
      getQuote: "Cotizar",
    },
    loading: {
      elevatingProjects: "Elevando tus proyectos...",
    },
  },
};

export type Language = keyof typeof translations;
export type TranslationKey = typeof translations.en;
